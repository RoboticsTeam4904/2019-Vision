...
std::optional<double> getDistance::getDistanceToWall(std::vector<cv::Point> &box, double distanceConstant = 96185.4, double gamma = 0) //gamma should be in radians
{
    cv::Point top = box[0];
    cv::Point bottom = box[2];
    double heightPixels = top.y - bottom.y;
    distanceConstant *= cos(gamma);
    std::optional<double> distanceToObject(distanceConstant/heightPixels * 0.03937);
                               ...
                               double distanceConstant = 96185.4, double gamma = 0)
{
    ...
    std::optional<double> distanceToObject(distance);
